...
// ConfidenceSlider — 0–100 with text labels and dynamic copy
  ...
  return (
    <div>
      <div style={{ position:'relative', height:14, borderRadius:999, background:'var(--bg-soft)', border:'1px solid var(--line)', overflow:'hidden' }}>
        <div style={{ position:'absolute', left:0, top:0, bottom:0, width:`${v}%`, background:'linear-gradient(90deg, var(--signal-soft), var(--signal))', transition:'width .12s ease' }}/>
        {/* 50% midpoint marker */}
        <div style={{ position:'absolute', left:'50%', top:0, bottom:0, width:2, background:'rgba(0,0,0,.18)' }}/>
        {[10,20,30,40,60,70,80,90].map(t => (
          <div key={t} style={{ position:'absolute', left:`${t}%`, top:0, bottom:0, width:1, background:'rgba(0,0,0,.08)' }}/>
        ))}
      </div>
      <input type="range" min="0" max="100" value={v} step="1" onChange={(e)=>onChange(+e.target.value)}
