...
const StationElection = ({ onComplete, recordScore }) => {
  ...
  const [log, setLog] = React.useState([]);    // {idx, prob}
  ...
  const advance = () => {
    setLog([...log, { reportedAfter: idx, prob }]);
    if (idx + 1 >= results.length) {
      ...
      // brier on FINAL forecast given outcome
      const brier = Math.pow(finalProb - (dWon ? 1 : 0), 2);
      // mean brier across all checkpoints
      const allUpdates = [...log, { reportedAfter: idx, prob }];
      const avgBrier = allUpdates.reduce((s,u)=>s + Math.pow(u.prob/100 - (dWon?1:0), 2), 0) / allUpdates.length;
      recordScore('election', { brier, avgBrier, dWon, totalD, totalR });
      setPhase('done');
      return;
    }
    setIdx(idx + 1);
    ...
    return (
      <Panel eyebrow="Simulation · Election Night" title={dWon ? 'D wins.' : 'R wins.'} accent={dWon ? 'var(--signal)' : 'var(--noise-2)'}>
        ...
        <div style={{ padding:'16px 18px', borderRadius:14, background:'var(--bg-soft)', border:'1px solid var(--line)' }}>
          <div className="mono" style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase', letterSpacing:'.14em', marginBottom:10 }}>Your P(D wins) through the night</div>
          <svg viewBox="0 0 480 160" width="100%" style={{ maxHeight:160 }}>
            ...
            <polyline fill="none" stroke="var(--signal)" strokeWidth="2.5"
              points={[...log, { reportedAfter: idx, prob }].map((u, i) => {
                const x = 30 + (i / (log.length)) * 430;
                const y = 140 - (u.prob / 100) * 130;
                return `${x},${y}`;
              }).join(' ')}/>
            {[...log, { reportedAfter: idx, prob }].map((u, i) => {
              const x = 30 + (i / (log.length)) * 430;
              const y = 140 - (u.prob / 100) * 130;
              return <circle key={i} cx={x} cy={y} r={3} fill="var(--signal)"/>;
            })}
            {/* truth marker */}
            <line x1="30" x2="460" y1={dWon?15:145} y2={dWon?15:145} stroke="var(--noise-2)" strokeWidth="1.5" strokeDasharray="2 3"/>
            <text x="460" y={dWon?12:158} fontSize="10" fill="var(--noise-2)" textAnchor="end" className="mono">truth = {dWon?'D':'R'} wins</text>
          </svg>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginTop:14 }}>
          <Stat label="Final P(D)" value={`${prob}%`} tone="signal"/>
          <Stat label="Final Brier" value={Math.pow(prob/100 - (dWon?1:0), 2).toFixed(3)} tone={Math.pow(prob/100 - (dWon?1:0), 2) < .15 ? 'good':'bad'}/>
          <Stat label="Avg Brier" value={([...log, { prob }].reduce((s,u)=>s+Math.pow(u.prob/100 - (dWon?1:0), 2), 0) / (log.length+1)).toFixed(3)} sub="across the night"/>
        </div>
        <Callout tone="signal" icon="◑">
...
const StationStock = ({ onComplete, recordScore }) => {
      ...
      <Panel eyebrow={`Simulation · Stock Picker · ${idx+1}/${StockPool.length}`} title={`${cur.ticker} closed ${moveStr}`} accent={last.win ? 'var(--leaf)' : 'var(--noise-2)'}>
        ...
        <div style={{ padding:'20px 24px', borderRadius:16, background: last.up ? 'var(--leaf-soft)':'var(--noise-soft)', border:`1.5px solid ${last.up ? 'var(--leaf)':'var(--noise)'}`, marginBottom:16, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            ...
            <div className="mono" style={{ fontSize:12, color:'var(--ink-3)', marginTop:4 }}>over the month</div>
          </div>
  ...
  return (
    <Panel eyebrow={`Simulation · Stock Picker · ${idx+1}/${StockPool.length}`} title={`${cur.ticker} · ${cur.name}`} accent="var(--signal)">
      ...
      <div style={{ marginTop:24, padding:'16px 18px', borderRadius:14, background:'var(--bg-soft)', border:'1px solid var(--line)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:6 }}>
          <span style={{ fontSize:14, color:'var(--ink-2)' }}>Bet size</span>
        ...
        <div className="mono" style={{ fontSize:11, color:'var(--ink-4)', marginTop:-2 }}>Kelly suggests roughly {Math.max(0, Math.round(Math.abs((prob/100)*2 - 1) * 100))}% on a fair-odds bet at your stated probability.</div>
      </div>
