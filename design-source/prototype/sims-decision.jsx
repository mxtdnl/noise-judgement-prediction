...
// ─── VC Portfolio ────────────────────────────────────────────────────────
// Allocate $10M across 8 startups. Each has signals + true 10-year IRR.
const VCPool = [
  { id:1, name:'Stratify',   sector:'B2B SaaS',     truthMult: 1.4,  signals:['Solo founder, 1st time','Decent traction','Crowded market'] },
  { id:2, name:'Holocene',   sector:'climate-tech', truthMult: 18.0, signals:['Two technical co-founders','Novel materials science','3 LOIs from majors'] },
  { id:3, name:'Pintail',    sector:'consumer',     truthMult: 0.0,  signals:['Hot at YC demo day','High burn','Reviews trend negative'] },
  { id:4, name:'Lazuli',     sector:'fintech',      truthMult: 3.2,  signals:['Strong technical co-founder','Regulated space','Mid traction'] },
  { id:5, name:'Vellichor',  sector:'consumer AI',  truthMult: 9.5,  signals:['Repeat founder (1 exit)','Real moat','Recent product-market fit'] },
  { id:6, name:'Drachma',    sector:'crypto',       truthMult: 0.4,  signals:['Trendy vertical','Unclear regulation','Founder turnover'] },
  { id:7, name:'Sundial',    sector:'devtools',     truthMult: 5.5,  signals:['Loved by power users','Slow but real growth','Open source moat'] },
  { id:8, name:'Marrow',     sector:'biotech',      truthMult: 0.0,  signals:['Promising preclinical','But FDA path unclear','Founder is a scientist, not an operator'] },
];
...
const StationVC = ({ onComplete, recordScore }) => {
  ...
  if (phase === 'intro') {
    return (
      <Panel eyebrow="Simulation · VC Portfolio" title="You have $10M to allocate across 8 startups." accent="var(--gold)">
        <Callout tone="signal" icon="◔">
          Each startup has visible signals — sector, team, traction notes. After your allocation, we'll resolve their 10-year multiples. Some will go to zero. A few will become unicorns. Your portfolio return is the weighted sum of multiples on your stakes.
        </Callout>
        <p style={{ color:'var(--ink-2)', marginTop:14, fontSize:15 }}>
          The lesson here is about power-law distributions: VC returns are dominated by the top 1-2 picks. Diversifying broadly captures the upside; concentrating wrong destroys the portfolio.
        </p>
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
  ...
  if (phase === 'reveal') {
    const portfolioMult = VCPool.reduce((s, c) => s + allocs[c.id] * c.truthMult, 0) / total;
    const portfolioValue = total * portfolioMult * 100; // back to dollars ($100 per unit)
    // optimal — concentrate on top mult names (but with diversification toward power-law)
    const sortedByMult = [...VCPool].sort((a,b)=>b.truthMult-a.truthMult);
    const optimalMult = (sortedByMult.slice(0,3).reduce((s,c)=>s+c.truthMult,0)/3); // crude proxy
    recordScore('vc', { portfolioMult, portfolioValue: portfolioValue/100, allocs, totalUnits: total });
    return (
      <Panel eyebrow="Simulation · VC Portfolio · Resolved · 10 years later" title={`Portfolio return: ${portfolioMult.toFixed(2)}×`} accent={portfolioMult > 2 ? 'var(--leaf)' : portfolioMult > 1 ? 'var(--gold)' : 'var(--noise-2)'}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14, marginBottom:16 }}>
          <Stat label="Multiple" value={`${portfolioMult.toFixed(2)}×`} tone={portfolioMult > 2 ? 'good' : portfolioMult > 1 ? 'neutral' : 'bad'}/>
          <Stat label="Value at exit" value={`$${(portfolioMult * 10).toFixed(1)}M`} tone={portfolioMult > 1 ? 'good':'bad'} sub={`from $10M`}/>
          <Stat label="Best possible" value={`${optimalMult.toFixed(2)}×`} sub="concentrated in winners"/>
        </div>
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--line)', borderRadius:14, overflow:'hidden' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr 1fr 1fr 1fr', padding:'10px 14px', background:'var(--bg-soft)', borderBottom:'1px solid var(--line)' }} className="mono">
            ...
            <span style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase', letterSpacing:'.14em' }}>10yr mult</span>
            <span style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase', letterSpacing:'.14em' }}>outcome</span>
            <span style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase', letterSpacing:'.14em', textAlign:'right' }}>returned</span>
          </div>
          {VCPool.map((c, i) => {
            const returned = allocs[c.id] * c.truthMult * 100;
            const stake = allocs[c.id] * 100;
            return (
              <div key={c.id} style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr 1fr 1fr 1fr', padding:'10px 14px', borderBottom: i < VCPool.length-1 ? '1px solid var(--line)':'none' }} className="mono">
                <span style={{ fontSize:13, fontWeight:500 }}>{c.name}</span>
                <span style={{ fontSize:13 }}>${(stake/1000).toFixed(0)}k</span>
                <span style={{ fontSize:13, color: c.truthMult === 0 ? 'var(--bad)' : c.truthMult >= 5 ? 'var(--good)' : 'var(--ink-3)' }}>{c.truthMult.toFixed(1)}×</span>
                <span style={{ fontSize:11, color: c.truthMult === 0 ? 'var(--bad)' : c.truthMult >= 5 ? 'var(--good)' : 'var(--ink-3)' }}>{c.truthMult === 0 ? 'failure' : c.truthMult >= 10 ? 'unicorn' : c.truthMult >= 3 ? 'success' : c.truthMult >= 1 ? 'modest' : 'underperform'}</span>
        ...
        <Callout tone="signal" icon="◆">
          <strong>Power-law lesson:</strong> a single 18× winner (Holocene) or 9.5× (Vellichor) typically pays for the whole portfolio plus more. If you concentrated on "safer-looking" picks like Lazuli (3.2×) or Sundial (5.5×) and missed the unicorns, your return is mediocre. If you bet on Pintail (0×) or Drachma (.4×), you donated capital.
        </Callout>
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:16 }}>
          <Button size="lg" onClick={onComplete}>Back to lab →</Button>
        </div>
  ...
  return (
    <Panel eyebrow="Simulation · VC Portfolio · Allocate" title={`Remaining: $${(remaining * 100 / 10000).toFixed(0)}M of $10M`} accent="var(--gold)">
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:18 }}>
        <span className="mono" style={{ fontSize:13, color:'var(--ink-3)' }}>{remaining === 0 ? '✓ Fully allocated' : `${(remaining*100/total).toFixed(0)}% unallocated (will be returned to LPs)`}</span>
        <Button size="lg" onClick={()=>setPhase('reveal')}>Lock portfolio · 10-year jump →</Button>
      </div>
    </Panel>
  );
};
...
const StationInspector = ({ onComplete, recordScore }) => {
  ...
  return (
    <Panel eyebrow="Simulation · Inspector · Tune" title="Score distributions" accent="var(--leaf)">
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
        <Button size="lg" onClick={()=>setPhase('done')}>Lock threshold →</Button>
      </div>
    </Panel>
  );
};
...
const WhistleCase = {
  ...
  rumors:[
    ...
    { id:5, source:'A board member privately expressed concerns over dinner', reliability:.85, isIndependent:true, fits:0.65, note:'Independent and credible source — but the board member only repeated rumors they\'d heard, not first-hand observation.' },
  ],
};

const StationWhistle = ({ onComplete, recordScore }) => {
  const [phase, setPhase] = React.useState('intro');
  const [evaluated, setEvaluated] = React.useState({}); // id -> { useIt, weight }
  const [finalProb, setFinalProb] = React.useState(50);

  // Compute "optimal" by treating non-independent evidence as 1 source
  const optimalPosterior = React.useMemo(() => {
    ...
    for (const r of independent) {
      // simplified LR: fits * reliability / (1-fits)*(1-reliability) — roughly Bayes factor
      const lr = (r.fits * r.reliability + (1-r.fits)*(1-r.reliability)) /
                 ((1-r.fits) * r.reliability + r.fits * (1-r.reliability));
      odds *= Math.max(lr, 0.4);
    }
    // for correlated group, count only the strongest
    if (correlatedGroup.length > 0) {
      const best = correlatedGroup.sort((a,b) => b.reliability * b.fits - a.reliability * a.fits)[0];
      const lr = (best.fits * best.reliability + (1-best.fits)*(1-best.reliability)) /
                 ((1-best.fits) * best.reliability + best.fits * (1-best.reliability));
      odds *= Math.max(lr, 0.4);
    }
    return odds / (1 + odds);
  }, []);
  ...
  if (phase === 'intro') {
    return (
      <Panel eyebrow="Simulation · Whistleblower" title={WhistleCase.claim} accent="var(--noise-2)">
        <Callout tone="signal" icon="⚠">
          You're an internal investigator. Five rumors have surfaced. Each has different reliability — and crucially, some are <strong>correlated</strong> (the sources talked to each other). You have to evaluate each, then commit to a probability the claim is true.
        </Callout>
        <p style={{ color:'var(--ink-2)', marginTop:12 }}>
          The trap: weighting correlated evidence as if it were independent will dramatically over-update your belief. The skill is tracing the source.
        </p>
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
    ...
    return (
      <Panel eyebrow="Simulation · Whistleblower · Resolved" title="How you weighted the evidence." accent="var(--noise-2)">
        ...
        <Callout tone={gap < .15 ? 'good' : gap < .30 ? 'gold' : 'noise'} icon={gap < .15 ? '✓' : gap < .30 ? '◑' : '✗'}>
          <strong>Gap from optimal: {Math.round(gap*100)} points.</strong> {gap < .15 ? 'Sharp evaluation — you correctly down-weighted the correlated evidence.' : gap < .30 ? 'Reasonable, but the correlated rumors likely got more weight than they should have.' : 'You probably treated rumors 3 and 4 as independent of rumor 2. They\'re not. Three "independent" sources who all heard the same thing count as roughly one.'}
        </Callout>
  ...
  // eval phase
  return (
    <Panel eyebrow="Simulation · Whistleblower · Evaluate" title="Five rumors. Five reliabilities. Some are correlated." accent="var(--noise-2)">
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {WhistleCase.rumors.map(r => (
          <div key={r.id} style={{ padding:'14px 16px', borderRadius:12, background:'var(--bg-card)', border:'1px solid var(--line)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline' }}>
      ...
      <div style={{ marginTop:18, padding:'16px 20px', borderRadius:14, background:'var(--bg-soft)', border:'1px solid var(--line)' }}>
        ...
        <div style={{ marginTop:8, color:'var(--ink-2)' }}>Given the rumors, your posterior P(claim is true)?</div>
        <ProbabilitySlider value={finalProb} onChange={setFinalProb} lowLabel="false" highLabel="true"/>
      </div>

      <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
        <Button size="lg" onClick={()=>setPhase('final')}>Lock posterior →</Button>
      </div>
    </Panel>
  );
};
